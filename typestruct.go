...
// 此結構主要用於模板渲染階段，提供完整的國際化（i18n）相關資訊，
// 包含套件名稱、鍵值對應資訊、多語系資料以及預設結構後綴。
// 各欄位需在進入模板前完成初始化，以確保模板渲染正確無誤。
...
// 通常用於在模板中生成對應的存取方法（getter），
// 以提供型別安全的方式取得翻譯內容。
type KeyMeta struct {
	Key        string // 原始翻譯鍵（例如："user.login.title"）
	MethodName string // 對應生成的方法名稱（例如："UserLoginTitle"）
}

// LocaleData 表示單一語系的翻譯資料。
// 每個語系會對應一組 key-value 的翻譯內容，
// 並可透過 StructSuffix 區分不同語系生成的結構名稱。
