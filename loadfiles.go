...
// 語系識別碼從檔名中擷取（例如 app_zh_TW.arb → zh_TW）。
...
// 會略過以 @ 開頭的 ARB 中繼資料欄位，並僅處理字串型別的翻譯值。
...
// toCamelCase 將底線或連字號分隔的字串轉為首字母大寫的 PascalCase 格式。
// 例如："hello_world" → "HelloWorld"，"zh-TW" → "ZhTW"。
// 注意：點號（.）不會被視為分隔符，因此 "user.login" → "User.login"。
